    ...
    <div class="box">
      ...
      <div class="content">
        ...
        <div class="mainBox">
          ...
          <keep-alive :include="keepComs" v-show="!w">
            <router-view/>
          </keep-alive>
          <weChat v-show="w"></weChat>
        </div>
      </div>
    </div>
    <div class="call" v-if="callShow">
      <div class="close" @click="closeCall">关闭</div>
...
export default {
  ...
  components:{
    leftMenu,statusBar,tabViews,weChat
  },
  methods:{
    accept(){
      if(this.$store.state.agentInfo.phoneType == "pstnPhone"){
        this.$message.info("请在PSTN或SIP电话端接听。");
        return;
      }
    ...
    reject(){
      if(this.$store.state.agentInfo.phoneType == "pstnPhone"){
        this.$message.info("请在PSTN或SIP电话端挂断。");
        return;
      }
      if (QPhoneTool.qPhone && QPhoneTool.qPhone.currentCall) {
        //来电时挂断
        this.$store.state.callOffIf = 1;
        QPhoneTool.qPhone.currentCall.Hangup();
      }
    },
    closeCall(){
      this.$confirm('是否确定关闭当前来电弹窗?', '提示', {
