  ...
  <div class="base">
    ...
    <div class="base_right" >
      <div class="base_right_top">
        <el-input class='fl' v-model="searchTitle" size="small" placeholder="标题" @keyup.enter.native='getAllarticle(1)'></el-input>
        <el-button class='fl' type="primary" size='small' @click="getAllarticle(1)">搜索</el-button>
      ...
      <div class="tab_wrap">
        <el-table
          :data="pagetableData"
          style="width:100%;height:calc(100% - 42px);background:#f0f1f6;overflow:auto"  class="article"  :show-header="false" :highlight-current-row="true" ref="tab"
          >
  ...
  name:'knowledgeBase',
    ...
    methods: {
      ...
      goarticledetail (data) {
        console.log("跳转文章详情",data);
        let id = data.id;
        this.$store.commit('getArticleId',id);
        this.$router.push({path:this.$route.meta.one+'/articleDetail'})
      },
      goAddarticle () {
        this.$router.push({path:this.$route.meta.one+'/addArticle'})
      },
      getAllarticle (s) {
        this.tableData = [];
        this.current = 1;
      ...
      initallArticle (s) {
        ...
        if(!that.akindId){
          ...
          for(let i=0;i<that.treedata.length;i++){
            this.$set(that.treedata[i],"selected",false);
          }
          //  this.akindId = '';
        }
        var url = "/knowledge/allKnowledges"
        if (s==1) {
          url = "/knowledge/findKnowledgesBy"
        }
        this.axios({
          method:'get',
          url:url,
      ...
      deleteArticle (data,index) {
        ...
        this.$confirm("您确定要删除‘"+data.title+"’这篇文章？","提示",{
          type:"warning"
            ...
            let data = res.data;
            if(status === 'success'){
              let inde = that.pagetableData.findIndex(el=>el.id == id);
              that.pagetableData.splice(inde,1);
              let inde1 = that.tableData.findIndex(el=>el.id == id);
      ...
      changeArticle (data) {
        ...
        this.$router.push({path:this.$route.meta.one+'/changeArticle'})
      },
      initAnnouncement (id)  {
        let that = this;
    ...
    computed:{
      ...
      addaArticleStatus () {
        return this.$store.state.addarticleStatus;
      }
    },
    mounted () {
      let that = this;
        ...
        handler(nwevalue,oldname){
          if(nwevalue){
            if(!this.akindId){
              ...
              this.tableData = [];
              // this.initKnowledgeKindAddStatus();
              this.initAnnouncement(this.akindId);
            }
          }
        },
        immediate:true
...
.base_right .base_right_top .fl:nth-child(1){
  width:200px;
  margin-right:5px;
}
</style>
